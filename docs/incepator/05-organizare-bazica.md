...
```mermaid
graph TD
    CHAOS["😵 Chaos<br/>2000 track-uri într-o grămadă"] -->|"Organizare"| ORDER["😎 Ordine<br/>Găsesc orice în 5 secunde"]
    
    ORDER --> GENRE["📁 Per Gen<br/>Techno, Manele..."]
    ORDER --> ENERGY["⚡ Per Energie<br/>Warmup → Peak"]
    ORDER --> MOOD["🎭 Per Mood<br/>Dark, Happy, Emotional"]
    ORDER --> SETS["🎧 Per Set<br/>Practice, Live"]
    
    style CHAOS fill:#ef4444,stroke:#dc2626,color:#fff
    style ORDER fill:#4ade80,stroke:#16a34a,color:#000
```
...
```mermaid
graph TD
    A["📥 Muzică nouă descărcată"] --> B["📂 Mută în H:\Music\_Inbox"]
    B --> C["📥 Import în rekordbox"]
    C --> D["🔬 Verifică analiză (BPM, Key)"]
    D --> E["🏷️ Setează taguri & rating"]
    E --> F["📁 Adaugă la playlist-ul corect"]
    F --> G["✅ Track organizat!"]

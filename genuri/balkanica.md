...
```mermaid
graph TD
    BALK["🌍 Muzică Balkanică"]
    BALK --> TURBO["Turbo Folk<br/>(Serbia, Bosnia)"]
    BALK --> BRASS["Balkan Brass<br/>(Fanfare, Guča style)"]
    BALK --> CHALGA["Chalga<br/>(Bulgaria)"]
    BALK --> SKILADIKO["Skiladiko<br/>(Grecia)"]
    BALK --> MOD["Balkan Beats<br/>(Modern, electronic mix)"]
    BALK --> RO["Muzică RO<br/>(Overlap manele+populară)"]
    
    style MOD fill:#a78bfa,stroke:#7c3aed,color:#000
```

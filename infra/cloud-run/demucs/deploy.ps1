# Deploy Demucs stem separation to Cloud Run GPU (L4) in europe-west4.
# Mirrors infra/cloud-run/ace-step/deploy.ps1.
[CmdletBinding()]
param(
    ...
    [string]$ServiceName = "mmo-demucs",
    [string]$ServiceAccount = "sa-demucs@mmo-mw-prod.iam.gserviceaccount.com",
    [string]$Memory = "16Gi",
    [string]$Cpu = "4",
    [int]$Concurrency = 1,
    [int]$Timeout = 900,
...
Push-Location $here
try {
    Write-Host "[demucs] Building $image ..." -ForegroundColor Cyan
    gcloud builds submit --project=$Project --region=$BuildRegion `
        --machine-type=e2-highcpu-32 --timeout=2400s `
        --tag=$image .
    if ($LASTEXITCODE -ne 0) { throw "build failed" }

    Write-Host "[demucs] Deploying $ServiceName to $Region ..." -ForegroundColor Cyan
    ...
    Write-Host ""
    Write-Host "GCP_DEMUCS_URL=$url" -ForegroundColor Green
} finally {
    Pop-Location
}

...
export async function getAnalysisScope(): Promise<AnalysisScope> {
    ...
    // Tracks analyzed in the last 7 days (for skip-recently-analyzed indicator)
    const sevenDaysAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();
    const [recentlyAnalyzedResult] = await db
        .select({ value: count() })
        .from(tracks)
        .where(sql`${tracks.analyzedAt} IS NOT NULL AND ${tracks.analyzedAt} >= ${sevenDaysAgo}`);
    ...
    return {
        ...
        recentlyAnalyzed: recentlyAnalyzedResult.value,
    };
}
